...
- feat: surface CS2 update-required warnings in the Servers UI
- feat: ingest `cs2_update_required` server webhook event and persist required CS2 version per server
- feat: add server reallocate endpoint + admin control to move staged matches off a bad server
- feat: show installed CS2 build per server and persist last fetched build/version string
- feat: report MatchZy plugin DB health via `server_health` and surface persistent admin warning
- feat: speed up server initialization via bootstrap payload (`/api/servers/:id/bootstrap`)
- feat: store server reachability proof (`server_can_reach_api_at`) from any successful server event
