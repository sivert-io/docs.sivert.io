...
description: A tour of the dashboard and what each sidebar section is for.
---
...
- **Settings**: Global settings for your installation (webhook URL, player registration, and MatchZy defaults).

> For details about what you can change in **Settings**, see: [Admin settings](/docs/mat/user/admin-settings)

### System tools

- **Admin Tools**: Operational tools for admins (for example maintenance and monitoring tools).
- **Dev Tools** (optional): Only visible in development mode, or when explicitly enabled with `VITE_ENABLE_DEV_PAGE=true`. These are **for developers**, not for production admins.
